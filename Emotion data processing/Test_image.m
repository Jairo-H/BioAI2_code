...
if testTransformer
    YTest = minibatchpredict(trainedNet,showData);
    classNames = categories(showData.Labels);
    YoutTest = onehotdecode(YTest,classNames,2);
else
...
[X1,Y1,R,AUC1]=perfcurve(RealAction, PredAction,1);
[X2,Y2,R,AUC2]=perfcurve(RealAffection, PredAffection,1);
...
if ifShowImg
    ...
    for i_label = 1:3:length(label_uniq)
        ...
        save(strcat("./SNE_before/map_",string(label_uniq(i_label)),"_x"),"map_x_i_label");
        save(strcat("./SNE_before/map_",string(label_uniq(i_label)),"_y"),"map_y_i_label");
        save(strcat("./SNE_before/map_",string(label_uniq(i_label)),"_z"),"map_i_label");
    end
