...
dataFile = './Dataset/'; % Header needs to be changed
NewDir = '/saveData/'; % Header needs to be changed
if save_train_signal
    experimentorSet = ["user1","user2","user3","user4","user5","user6","user7","user8","user9","user10","user11","user12"];
else
    experimentorSet = ["user13"]; % use user 13 for testing
end
...
name_num = 0;
ii = 1;
imageRoot = '.\All_Data\datasetImage\ImageNorm_Force_30_only';
experimentNum = 0;
...
function [o2,o3,maxIm] = saveImage(V_Seg,label,imageRoot)
    ...
    % Calculate the segmented frequency for every second
    num_thousand = floor(length(V_Seg)/1000);
    fre_sec = zeros(num_thousand,1);
    tmp_num = 0;

    % the data type and their scale here can be changed according to image's requirement
